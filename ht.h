...
// Almost Complete - Fill in the if statement below. --  COMPLETE??
template <typename KeyType>
...
private:
    ...
    size_t numDeleted_; // number of deleted items in the table
...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
       :  hash_(hash), kequal_(kequal), prober_(prober), resizeAlpha_(resizeAlpha), 
            mIndex_(0), numItems_(0), numDeleted_(0), totalProbes_(0)
{
...
{
    double loadFactor = double(numItems_ + numDeleted_) / double(CAPACITIES[mIndex_]);
